          ...
          <div style={{ background: T.sand, overflowY: "auto" }}>
            ...
            {status === "concluido" && (
              <div style={{ maxWidth: 700, margin: "0 auto", padding: "24px 20px" }}>
                {/* Banner */}
                <div style={{
                  background: T.ink, borderRadius: 12, padding: "16px 20px", marginBottom: 20,
                  ...
                  <div>
                    <div style={{ fontSize: 13, fontWeight: 700, color: "#fff" }}>
                      Projeto concluído
                      {approvedChoice ? ` · ${approvedChoice.label} aprovada` : ""}
                    </div>
                ...
                {approvedChoice && (
                  <div style={{
                    background: T.bg, border: `2px solid ${T.lime}`, borderRadius: 12, padding: 12, marginBottom: 16,
                  }}>
                    {approvedChoice.image_url && (
                      <div style={{ borderRadius: 8, overflow: "hidden", marginBottom: 10, border: `1px solid ${T.border}` }}>
                        <img src={approvedChoice.image_url} alt="" style={{ width: "100%", height: 200, objectFit: "cover", display: "block" }} />
                      </div>
                    )}
                    <div style={{
                      background: T.sand, border: `1px solid ${T.border}`, borderRadius: 8,
                      ...
                      <button
                        id="copy-final"
                        onClick={() => copiar(approvedChoice.content?.text || "", "copy-final")}
                        style={{
                          position: "absolute", top: 8, right: 8, fontSize: 11, fontWeight: 600,
                          color: T.muted, background: T.bg, border: `1px solid ${T.border}`,
                          borderRadius: 6, padding: "3px 10px", cursor: "pointer",
                    ...
                    <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
                      <button
                        onClick={() => downloadPDF(approvedChoice)}
                        style={{
                          flex: 1, padding: "10px", borderRadius: 8, fontSize: 12, fontWeight: 600,
                          background: T.lime, border: "none", color: T.ink, cursor: "pointer", fontFamily: FI,
                        }}
                      >↓ Baixar novamente</button>
                      <a
                        href="/cliente/projetos/novo"
                        style={{
                          flex: 1, padding: "10px", borderRadius: 8, fontSize: 12, fontWeight: 600,
                          background: "transparent", border: `1px solid ${T.border}`, color: T.ink,
                          cursor: "pointer", fontFamily: FF, textAlign: "center", textDecoration: "none",
                          display: "flex", alignItems: "center", justifyContent: "center",
                        }}
                      >Solicitar nova versão</a>
                    </div>
                  </div>
                )}

                {/* Other choices (non-approved) */}
                {choices.filter(c => !c.is_selected).length > 0 && (
                  <div style={{ marginTop: 16 }}>
                    <p style={{ fontSize: 12, fontWeight: 600, color: T.muted, marginBottom: 8 }}>Outras opções geradas</p>
                    {choices.filter(c => !c.is_selected).map(c => (
                      <div key={c.id} style={{
                        background: T.bg, border: `1px solid ${T.border}`, borderRadius: 10,
                        padding: "8px 12px", marginBottom: 6, opacity: 0.6,
                      }}>
