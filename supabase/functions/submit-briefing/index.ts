...
const PRODUCTS: Record<string, { name: string; deadline_hours: number; usd: number; brl: number }> = {
  landing_page_copy: { name: "Landing Page Copy", deadline_hours: 24, usd: 97, brl: 497 },
  content_pack: { name: "Pacote de Conteúdo para Redes", deadline_hours: 48, usd: 147, brl: 747 },
  email_sequence: { name: "Sequência de E-mails de Nutrição", deadline_hours: 48, usd: 127, brl: 647 },
};
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { user_id, email, name, product, conversation, structured_data, currency = "USD" } = body;
    ...
    if (!productInfo) {
      return new Response(JSON.stringify({ error: "Produto inválido" }), { status: 400 });
    }
      ...
      body: JSON.stringify({
        ...
        html: `
          <div style="font-family: monospace; background: #0A0A0A; color: #F0F0EC; padding: 40px; max-width: 560px; margin: 0 auto; border-radius: 12px;">
            ...
            <p style="color: #555; font-size: 12px;">Você receberá outro e-mail assim que o arquivo estiver pronto para download.</p>
